...
const akuariapi = require('./akuariapi')
...
const animesearch = async (title) => {
  const APIURL = `https://api.jikan.moe/v4/anime?q=${title}`;
  let data, picUrl, caption
  try {
    data = await axios.get(APIURL);
    const result = data.data.data[0];
    picUrl = result.images.jpg.large_image_url;
    caption = `_*Judul* : ${result.title}_\n_*Genre* : ${result.genres.map(u => u.name).join(', ')}_\n*Sinopsis* : ${result.synopsis}\n_*Episodes* : ${result.episodes}_\n_*Rated* : ${result.rating}_\n_*Score* : ${result.score}_\n_*Studio* : ${result.studios[0].name}_\n_*Link* : ${result.url}_`;
  } catch (error) {
